...
for(i in 1:nLoops){
  ...
  data.sets <- list(Cleve[OOB,], data.v, data.t)
  
  pp1 <- lapply(1:3, 
              function(i) predict(forest , data=data.sets[[i]] , predict.all = T)$predictions)
  ...
  if(sizeSF>1){
    ...
    LL <- lapply(1:3,
               function(j)
                 lapply(idcsList 
                        , function(i) unlist(calcLogloss2(pp=apply(pp1[[j]][,2,i],1,mean) 
                                                        , df=data.sets[[j]] ) ) )
               )}else{
      # no need for a mean with only one prediction (sizeSF==1 : one tree in forest)
      LL <- lapply(1:3,
                   function(j)
                     lapply(idcsList 
                            , function(i) unlist(calcLogloss2(pp=pp1[[j]][,2,i] 
                                                              , df=data.sets[[j]] ) ) )
                   )}
  
  if(sizeSF>1){
  lapply(idcsList , function(i) calcLogloss2(pp=apply(pp1[[1]][,2,i],1,mean) , df=data.sets[[1]])) %>% 
    unlist -> LL.OOB # logloss on OOB of simulation
  
  lapply(idcsList , function(i) calcLogloss2(pp=apply(pp1[[2]][,2,i],1,mean) , df=data.sets[[2]])) %>% 
    unlist -> LL.val # logloss on validation set
  
  lapply(idcsList , function(i) calcLogloss2(pp=apply(pp1[[3]][,2,i],1,mean) , df=data.sets[[3]])) %>% 
    unlist -> LL.test # logloss on test set
  }else{
    lapply(idcsList , function(i) calcLogloss2(pp=pp1[[1]][,2,i] , df=data.sets[[1]])) %>% 
      unlist -> LL.OOB # logloss on OOB of simulation
    
    lapply(idcsList , function(i) calcLogloss2(pp=pp1[[2]][,2,i] , df=data.sets[[2]])) %>% 
      unlist -> LL.val # logloss on validation set
    
    lapply(idcsList , function(i) calcLogloss2(pp=pp1[[3]][,2,i] , df=data.sets[[3]])) %>% 
      unlist -> LL.test # logloss on test set
  }
  ...
  if(i%%2 ==0){
    ...
    #boxplot(as.data.frame(LL))
    boxplot(cbind(unlist(LL[[1]]),unlist(LL[[2]]),unlist(LL[[3]])))
  }
